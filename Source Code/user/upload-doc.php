...
if(isset($_POST['submit']))
  {
    $uid=$_SESSION['uid'];
		...
		$eemarksheet= $_FILES["eeimage"]["name"];
...
$extensioneem = substr($eemarksheet,strlen($eemarksheet)-4,strlen($eemarksheet));
...
elseif(!in_array($extensioneem,$allowed_extensions))
{
echo "<script>alert('Invalid format. Only jpg / jpeg/ png /gif/pdf format allowed');</script>";
}
...
$eem=md5($eemarksheet).$extensioneem;
if($gramarkshee!=""){
...
$pgra=md5($pgmarksheet).$extensionpgra;
	 ...
	 move_uploaded_file($_FILES["eeimage"]["tmp_name"],"userdocs/".$eem);
     move_uploaded_file($_FILES["graimage"]["tmp_name"],"userdocs/".$gra);
     move_uploaded_file($_FILES["pgraimage"]["tmp_name"],"userdocs/".$pgra);
    $query=mysqli_query($con,"insert into tbldocument(UserID,TransferCertificate,TenMarksheeet,TwelveMarksheet,EEMarksheet,GraduationCertificate,PostgraduationCertificate) 
	value('$uid','$tc','$tm','$twm','$eem','$gra','$pgra')");
    if ($query) {
...
<tr>
  ...
  <td><a href="userdocs/<?php echo $row['EEMarksheet'];?>" target="_blank">View File </a></td>
</tr>
...
<div class="col-xl-6 col-lg-12">
  <fieldset>
  <h5>Entrance Exam Marksheet                </h5>
   <div class="form-group">
    <input class="form-control white_bg" id="eeimage" name="eeimage"  type="file">
    </div>
</fieldset>
 </div>
